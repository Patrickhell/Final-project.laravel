...
class DoctorController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!empty($specialtyNames)) {
            $specialtiesArray = explode(',', $specialtyNames);  // Convert comma-separated string to array
            $query->whereHas('specialties', function ($query) use ($specialtiesArray) {
                $query->whereIn(DB::raw('LOWER(name)'), array_map('strtolower', $specialtiesArray));
            });
        }
